...
parfor jj=1:66
    % Assumes asymptomatic enter over infections period [0,ts+20]
        IDSL(jj)=(1-pA).*((1./ts).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj),R0S,R0A,0,ts,tLv(jj),SelfIsolate),0,ts,q(jj),inf))+pA.*((1./td).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj),R0S,R0A,1,ts,tLv(jj),0),0,td,q(jj),inf));  
end
