function R = InfectiousnessfromInfectionTestingEntryExit(t,u,ux,R0S,R0A,pA,ts,td,tL,SelfIsolate)
...
SA = SensitivityvsViralLoad(ViralShedding_Asymptomatic(u,tL,Stdv),ts,tL,Stdv).*ones(size(t));
%SA(t-u<0)=0; % Should not occur as we feed in t+u but have condition just in case
SAx = SensitivityvsViralLoad(ViralShedding_Asymptomatic(ux,tL,Stdv),ts,tL,Stdv).*ones(size(t));
...
SS = SensitivityvsViralLoad(ViralShedding_Symptomatic(u,tL,Stdv),ts,tL,Stdv).*ones(size(t));
%SS(t-u<0)=0; % Should not occur as we feed in t+u but have condition just in case
SSx = SensitivityvsViralLoad(ViralShedding_Symptomatic(ux,tL,Stdv),ts,tL,Stdv).*ones(size(t));
