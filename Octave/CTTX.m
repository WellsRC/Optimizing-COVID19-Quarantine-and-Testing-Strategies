...
parfor jj=1:66
        ...
        R(jj)=dblquad(@(u,t)(DurationInfected(u,IncubationI(jj),ts,tLv(jj)).*InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj),R0S,R0A,pA,ts,tLv(jj),SelfIsolate)),0,ts,q(jj),inf);       
        RD1(jj)=dblquad(@(u,t)(DurationInfected(u,IncubationI(jj),ts,tLv(jj)).*InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj)+d1,R0S,R0A,pA,ts,tLv(jj),SelfIsolate)),0,ts,q(jj)+d1,inf);
        
        % Computation for asymptomatic that infected the index case        
        % ONLY ASYMPTOMATIC (SET SELF-ISOLATION TO ZER0 TO ENSURE)
        RA(jj)=(1./quad(@(u)(InfectiousnessfromInfection(u,R0,R0,1,ts,tLv(jj),0)),ts,inf)).*dblquad(@(u,t)(InfectiousnessfromInfection(u,R0S,R0A,1,ts,tLv(jj),0).*InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj),R0S,R0A,1,ts,tLv(jj),0)),ts,inf,q(jj),inf);       
        RAD1(jj)=(1./quad(@(u)(InfectiousnessfromInfection(u,R0,R0,1,ts,tLv(jj),0)),ts+d1,inf)).*dblquad(@(u,t)(InfectiousnessfromInfection(u,R0S,R0A,1,ts,tLv(jj),0).*InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj),R0S,R0A,1,ts,tLv(jj),0)),ts+d1,inf,q(jj),inf);            
end
