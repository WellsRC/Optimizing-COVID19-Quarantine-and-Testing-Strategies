% Conducts the analysis for contact tracing and testing on exit
clear;
...
parfor jj=1:4221
        ...
        R(jj)=integral2(@(u,t)(DurationInfected(u,IncubationI(jj),tsv(jj)).*InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj)-1,R0S,R0A,pA,tsv(jj),SelfIsolate)),0,tsv(jj),q(jj),inf);       
        RD1(jj)=integral2(@(u,t)(DurationInfected(u,IncubationI(jj),tsv(jj)).*InfectiousnessfromInfectionTestingEntry(t+u,u+d1+q(jj)-1,R0S,R0A,pA,tsv(jj),SelfIsolate)),0,tsv(jj),q(jj)+d1,inf);
        
        % Computation for asymptomatic that infected the index case
        % ONLY ASYMPTOMATIC (SET SELF-ISOLATION TO ZER0 TO ENSURE)
        RA(jj)=(1./integral(@(u)(InfectiousnessfromInfection(u,R0,R0,1,tsv(jj),0)),tsv(jj),inf)).*integral2(@(u,t)(InfectiousnessfromInfection(u,R0S,R0A,1,tsv(jj),0).*InfectiousnessfromInfectionTestingEntry(t+u,u+q(jj)-1,R0S,R0A,1,tsv(jj),0)),0,td(jj),tsv(jj)+q(jj),inf);       
        RAD1(jj)=(1./integral(@(u)(InfectiousnessfromInfection(u,R0,R0,1,tsv(jj),0)),tsv(jj)+d1,inf)).*integral2(@(u,t)(InfectiousnessfromInfection(u,R0S,R0A,1,tsv(jj),0).*InfectiousnessfromInfectionTestingEntry(t+u,u+d1+q(jj)-1,R0S,R0A,1,tsv(jj),0)),0,td(jj),tsv(jj)+q(jj)+d1,inf);       
end
  
% Compute the total number of secondary infections
RTot=(PreI.*R+pA.*R0.*RA)./(PreI+pA.*R0);
RTotD1=(PreI.*RD1+pA.*R0.*RAD1)./(PreI+pA.*R0);
save('ContactTracing_IndexInfect_Contact_Test_Exit_OneDayDelay.mat');
...
save('ContactTracing_IndexInfect_Contact_Test_Exit_pA=226_OneDayDelay.mat');