...
parfor jj=1:63
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pAv(jj)).*((1./ts).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+qt-1,R0S,R0A,0,ts,tLv(jj),SelfIsolate),0,ts,qt,inf))+pAv(jj).*((1./td).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+qt-1,R0S,R0A,1,ts,tLv(jj),0),0,td,qt,inf));  
end
...
parfor jj=1:63
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pAv(jj)).*((1./ts).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+4,R0S,R0A,0,ts,tLv(jj),SelfIsolate),0,ts,qt,inf))+pAv(jj).*((1./td).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+4,R0S,R0A,1,ts,tLv(jj),0),0,td,qt,inf));  
end
