...
parfor jj=1:66
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pA).*((1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTestingEntry(t,u,R0S,R0A,0,ts,tLv(jj),SelfIsolate),0,ts,@(u)(u+q(jj)),inf))+pA.*((1./td).*integral2(@(u,t)InfectiousnessfromInfectionTestingEntry(t,u,R0S,R0A,1,ts,tLv(jj),0),0,td,@(u)(u+q(jj)),inf));  
end
