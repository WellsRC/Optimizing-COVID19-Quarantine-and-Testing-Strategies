...
for jj=1:4221
        ...
        IDS(jj)=(1./tsv(jj)).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+q(jj)-1,R0S,R0A,pA,tsv(jj),SelfIsolate),0,tsv(jj),q(jj),inf);   
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pA).*((1./tsv(jj)).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+q(jj)-1,R0S,R0A,0,tsv(jj),SelfIsolate),0,tsv(jj),q(jj),inf))+pA.*((1./td(jj)).*dblquad(@(u,t)InfectiousnessfromInfectionTestingEntryExit(t+u,u,u+q(jj)-1,R0S,R0A,1,tsv(jj),0),0,td(jj),q(jj),inf));  
end
