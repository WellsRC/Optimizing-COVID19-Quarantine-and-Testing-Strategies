...
parfor jj=1:66   
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pA).*(integral(@(t)InfectiousnessfromInfectionTestingEntry(t,0,R0S,R0A,0,ts,tLv(jj),SelfIsolate),q(jj),inf))+pA.*(integral(@(t)InfectiousnessfromInfectionTestingEntry(t,0,R0S,R0A,1,ts,tLv(jj),0),q(jj),inf));  
end

save('Known_Infection_TestingonEntry.mat');
...
%         IDS(jj)=(1./ts).*integral2(@(u,t)InfectiousnessfromInfectionTestingEntry(t,u,R0S,R0A,pA,ts,tLv(jj),SelfIsolate),0,ts,@(u)(u+q(jj)),inf);   
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pA).*(integral(@(t)InfectiousnessfromInfectionTestingEntry(t,0,R0S,R0A,0,ts,tLv(jj),SelfIsolate),q(jj),inf))+pA.*(integral(@(t)InfectiousnessfromInfectionTestingEntry(t,0,R0S,R0A,1,ts,tLv(jj),0),q(jj),inf));  
end
save('Known_Infection_TestingonEntry_pA=226.mat');
