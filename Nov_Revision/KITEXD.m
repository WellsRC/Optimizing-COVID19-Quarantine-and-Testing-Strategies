...
for jj=1:63 
    % Assumes asymptomatic enter over infections period [0,ts+21]
        IDSL(jj)=(1-pA).*(integral(@(t)InfectiousnessfromInfectionTestingEntryExit(t,0,q(jj)-1,R0S,R0A,0,ts,tLv(jj),SelfIsolate),q(jj),inf))+pA.*(integral(@(t)InfectiousnessfromInfectionTestingEntryExit(t,0,0+q(jj)-1,R0S,R0A,1,ts,tLv(jj),0),(q(jj)),inf));  
end

save('Known_Infection_TestingonEntryExit_OneDayDelay.mat');
...
save('Known_Infection_TestingonEntryExit_pA=226_OneDayDelay.mat');
