...
for jj=1:length(tsv)
    ...
    RNS(jj)=integral(@(t)InfectiousnessfromInfection(t,R0S,R0A,pA,ts,SelfIsolate),0,inf);

    SelfIsolate=1; % Self-isolation of symptomatics
    IDS(jj,:)=InfectiousnessfromInfection(t,R0S,R0A,pA,ts,SelfIsolate);
    RS(jj)=integral(@(t)InfectiousnessfromInfection(t,R0S,R0A,pA,ts,SelfIsolate),0,inf);
    
    RPre(jj)=integral(@(t)InfectiousnessfromInfection(t,R0S,R0A,pA,ts,SelfIsolate),0,ts);
end
