...
parfor jj=1:4422
        ...
        R(jj)=integral2(@(u,t)(DurationInfected(u,IncubationI(jj),tsv(jj)).*InfectiousnessfromInfection(t+u,R0S,R0A,pA,tsv(jj),SelfIsolate)),0,tsv(jj),(q(jj)),inf);       
        RD1(jj)=integral2(@(u,t)(DurationInfected(u,IncubationI(jj),tsv(jj)).*InfectiousnessfromInfection(t+u,R0S,R0A,pA,tsv(jj),SelfIsolate)),0,tsv(jj),(q(jj)+d1),inf); 
        
        % Computation for asymptomatic that infected the index case
        % ONLY ASYMPTOMATIC (SET SELF-ISOLATION TO ZER0 TO ENSURE)
        RA(jj)=(1./integral(@(u)(InfectiousnessfromInfection(u,R0,R0,1,tsv(jj),0)),tsv(jj),inf)).*integral2(@(u,t)(InfectiousnessfromInfection(u,R0S,R0A,1,tsv(jj),0).*InfectiousnessfromInfection(t+u,R0S,R0A,1,tsv(jj),0)),0,td(jj),tsv(jj)+q(jj),inf);       
        RAD1(jj)=(1./integral(@(u)(InfectiousnessfromInfection(u,R0,R0,1,tsv(jj),0)),tsv(jj)+d1,inf)).*integral2(@(u,t)(InfectiousnessfromInfection(u,R0S,R0A,1,tsv(jj),0).*InfectiousnessfromInfection(t+u,R0S,R0A,1,tsv(jj),0)),0,td(jj),tsv(jj)+q(jj)+d1,inf); 
end
