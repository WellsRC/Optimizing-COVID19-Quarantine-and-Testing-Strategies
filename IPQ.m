...
parfor jj=1:4422  
    % Assumes asymptomatic enter during incubation period
         IDS(jj)=(1./tsv(jj)).*integral2(@(u,t)InfectiousnessfromInfection(t,R0S,R0A,pA,tsv(jj),SelfIsolate),0,tsv(jj),@(u)(u+q(jj)),inf);   
    % Assumes asymptomatic enter over infections period [0,ts+21]
         IDSL(jj)=(1-pA).*((1./tsv(jj)).*integral2(@(u,t)InfectiousnessfromInfection(t,R0S,R0A,0,tsv(jj),SelfIsolate),0,tsv(jj),@(u)(u+q(jj)),inf))+pA.*((1./td(jj)).*integral2(@(u,t)InfectiousnessfromInfection(t,R0S,R0A,1,tsv(jj),0),0,td(jj),@(u)(u+q(jj)),inf));
end
